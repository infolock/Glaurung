...
- (void)startEngine {
  ...
  if ([[Options sharedOptions] strength] == 2450) // Max strength
    [engineController
...
- (void)updateMoveList {
  ...
  if ([game atEnd]) {
    ...
    if ([moveListView contentSize].height > 60)
      [moveListView
        setContentOffset:
          CGPointMake(0.0f, [moveListView contentSize].height - 63)];
  }
  else {
...
- (int)pieceCanMoveFrom:(Square)fSq to:(Square)tSq {
  ...
  // the side when castling, but Glaurung internally encodes castling moves
...
- (void)putPiece:(Piece)p on:(Square)sq {
  ...
  rect.origin = CGPointMake((int(sq)%8) * 40.0f, (7-int(sq)/8) * 40.0f);
...
- (void)takeBackMove {
  if (![game atBeginning]) {
    ...
    // HACK: Castling. Glaurung internally encodes castling moves as "king
...
- (void)showPiecesAnimate:(BOOL)animate {
  ...
  for (Square sq = SQ_A1; sq <= SQ_H8; sq++) {
    ...
    if (p != EMPTY) {
      assert(piece_is_ok(p));
      rect.origin = CGPointMake((int(s)%8) * 40.0f, (7-int(s)/8) * 40.0f);
      PieceImageView *piv = [[PieceImageView alloc] initWithFrame: rect
                                                   gameController: self
                                                        boardView: boardView];
      [piv setImage: pieceImages[p]];
...
- (void)playClickSound {
  if (![[[Options sharedOptions] soundVolume] isEqualToString: @"Off"])
...
- (void)showBookMoves {
  if ([[Options sharedOptions] showBookMoves]) {
    ...
    if (s)
      [analysisView setText: [NSString stringWithFormat: @"  Book: %@",
                                       [game bookMovesAsString]]];
    else if ([[analysisView text] hasPrefix: @"  Book:"])
      [analysisView setText: @""];
  }
  else if ([[analysisView text] hasPrefix: @"  Book:"])
    [analysisView setText: @""];
